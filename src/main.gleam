...
import shared_types/actor_pool_types.{ActorPool}
import shared_types/distributor_types.{DistributorMessage}
...
pub fn main() {
  ...
  let actor_pool =
    actor_pool.spawn_pool_of_memory_actors(
      memory_shard_count,
      ActorPool(size: 0, actors: dict.new()),
    )

  let distributor = distributor.spawn_message_distributor(actor_pool)
